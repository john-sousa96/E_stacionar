            ...
            if (c.connect() == true)
            {
                ...
                try
                {
                    ...
                    if (selectedIndex == 1)
                    {
                        ...
                        sql = "Select U.id_nota_fiscal_uso, U.valor_servico_uso, U.timestamp_inicio_uso, U.timestamp_final_uso, (DATEDIFF(hour, U.timestamp_inicio_uso, CURRENT_TIMESTAMP) * U.valor_servico_uso) as total, C.nome_usuario, S.desc_servico,v.local_vaga, R.id_placa_veiculo from tbl_uso as U inner join tbl_usuario as C on U.tbl_usuario_id_CPF_usuario = C.id_CPF_usuario inner join tbl_servico as S on U.tbl_servico_id_servico = S.id_servico inner join tbl_vaga as V on U.tbl_vaga_id_vaga = V.id_vaga inner join tbl_veiculos as R on U.tbl_token_id_token = R.tbl_token_id_token where timestamp_final_uso is null";
                    ...
                    {
                        btn_Consultar_Veiculo.Visible = false;
                        sql = "Select U.id_nota_fiscal_uso, U.valor_servico_uso, U.timestamp_inicio_uso, U.timestamp_final_uso, (DATEDIFF(hour, U.timestamp_inicio_uso, U.timestamp_final_uso) * U.valor_servico_uso) as total, C.nome_usuario, S.desc_servico,v.local_vaga, R.id_placa_veiculo from tbl_uso as U inner join tbl_usuario as C on U.tbl_usuario_id_CPF_usuario = C.id_CPF_usuario inner join tbl_servico as S on U.tbl_servico_id_servico = S.id_servico inner join tbl_vaga as V on U.tbl_vaga_id_vaga = V.id_vaga inner join tbl_veiculos as R on U.tbl_token_id_token = R.tbl_token_id_token where datediff(hour, timestamp_final_uso, CURRENT_TIMESTAMP) <= 24";
                        c.command.CommandText = sql;
